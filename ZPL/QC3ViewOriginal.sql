create or replace view mrlv_etiqc3tech_validade as
   select a.nroempresa,
       ...
       -- desta em fundo preto
           '^XA^DFR:FMT1.ZPL^FS
       ^LRY
       ^FO260,50^GB350,203,195^FS
       ^FO265,55^A0N,100,50^FN1^FS
       ^ LRN
       ^FO290,160^A0N,100,50^FN2^FS
       ^ LRN
       ^FT0,94^A0B,20,19^FH\^FN3^FS
       ^ LRN
       ^FT0,150^A0B,20,19^FH\^FN4^FS
       ^XZ

       ^XA^XFR:FMT1.ZPL^FS
       ^FN1^FDProduto proximo^FS
       ^FN2^FDao vencimento^FS'
       /*^FN3^FD'||a.seqproduto||'^FS*/
       /*^FN4^FDLJ: '||c.nroempresa||'^FS'*/
          ||
       -- codigo barra
           '^BY2,2,38^FT235,250^BEB,,Y,N^FD'
          ||
          case
